...
// Import Components
import { LabelData } from "./OrderManager";
// Import Services
    ...
    <div className="card my-3">
      ...
      <div className="card-content">
        <div className="content">
          ...
          <div className="is-flex is-align-items-baseline my-2">
            <span className="has-text-weight-bold mx-5">Lotes:</span>
            <p>
              {order.lot
                ? order.lot
                    .split(" ")
                    .map((l, idx) => <span key={idx}>{l}</span>)
                : "-"}
            </p>
          </div>
          ...
          <div className="is-flex is-align-items-baseline my-2">
            ...
            <ul>
              {order.products.map((p) => (
                <li key={p.id} className="tag is-black is-flex">
                  {p.name}
                </li>
              ))}
            </ul>
          ...
          />
          <LabelData label="Peso Neto (kg):" value={order.net_weight ?? "-"} />
