...
const CheckerModal: FC<CheckerProps> = ({
    ...
    :<>
        <div>
            ...
            <span>{labelOk}</span>
            {
                onChange && <ModalTrigger 
                    button={<span 
                                className="is-size-7 is-block has-text-danger is-clickable"
                            >
                                {labelWrong}
                            </span>} 
                    modal={
...
const CheckerLink: FC<CheckerLinkProps> = ({
    ...
    :<>
        <div>
            ...
            <Link 
                to={goUrl} 
                className="is-block is-size-7 has-text-danger"
            >
                {labelWrong}
            </Link>
        </div>
...
const ContainerResume: FC<ContainerProps> =({
    order,
    onOk
}) => {
...
const ContainerCloseResume: FC<ContainerProps> =({
    order,
    onOk
}) => {
...
const RowsResume: FC<ContainerProps & {rows: Row[]}> =({
    ...
    const checkRows = order.boxes === rows.reduce((a: number, c: Row) => a + c.quantity, 0);
    const maxRow = rows.reduce((a: number, c: Row) => a > c.number?a:c.number, 0);
            ...
            <ul className="mx-2">
                ...
                <li className="is-flex is-justify-content-space-between">
                    <CheckerModal
                        ...
                        labelWrong={"El número de cajas no coincide con el cargado en las filas."}
                        type="number"
                        name="boxes"
        ...
        return (
            <div>
                ...
                <div className="buttons is-justify-content-space-evenly">
                    ...
                    <Link to={backUrl} className="button is-danger is-large">Volver</Link>
                </div>
